...
from pyspark.sql.functions import col, create_map, lit, when, regexp_replace
...
@transform_df(
    ...
    measurements_all=Input("/{PATH}/measurement"),
    codeset_lookup=Input("/{PATH}/concept_set_members"),
    maps=Input("/{PATH}/unit_mappings"),
    newunits=Input("/{PATH}/inferred_units"),
    minsmaxs=Input("/{PATH}/canonical_units_of_measure"),
)
def my_compute_function(measurements_all, codeset_lookup, newunits, minsmaxs, maps):
    ...
    measurements_all = measurements_all.repartition(300)
    ...
    # remove any value from the original units that corresponds to null or nmc (the idea of this is to make anything 
    ...
    df =  df.withColumn("unit_concept_name",
            when(col('unit_concept_id').isNull(),None)
    ...
    df = df.join(
        maps.select("original_measurement_concept_id", "original_unit_concept_id", "harmonized_unit_concept_id", "map_function")
        .hint("broadcast"),
    ...
    df = df.withColumn("unit_concept_id_or_inferred_unit_concept_id", df["unit_concept_id_or_inferred_unit_concept_id"].cast(IntegerType()))
